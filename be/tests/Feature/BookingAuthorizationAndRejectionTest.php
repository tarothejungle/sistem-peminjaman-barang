...
final class BookingAuthorizationAndRejectionTest extends TestCase
{
    ...
    public function test_kasubag_rejection_requires_reason_before_database_access(): void
    {
        $bookingId = fake()->uuid();
    ...
    public function test_approval_cannot_inject_an_unchecked_alternative_schedule(): void
    {
        $this->withToken($this->token(Role::KASUBAG_UMUM))->patchJson('/api/v1/bookings/'.fake()->uuid().'/kabag-approve', [
            'status' => BookingStatus::APPROVED->value,
            'alternativeRoomId' => fake()->uuid(),
            'alternativeStartTime' => now()->addDay()->toIso8601String(),
            'alternativeEndTime' => now()->addDays(30)->toIso8601String(),
        ])->assertBadRequest()->assertJsonPath('error.message', 'Data tidak valid');
    }

    public function test_kabag_is_read_only_on_the_approval_queue(): void
    {
        $bookingId = fake()->uuid();
        $kabag = $this->token(Role::KABAG_UMUM);

        foreach ([BookingStatus::APPROVED->value, BookingStatus::REJECTED->value] as $status) {
            $this->withToken($kabag)->patchJson("/api/v1/bookings/{$bookingId}/kabag-approve", [
                'status' => $status,
                'rejectionReason' => 'Stok tidak tersedia',
            ])->assertForbidden();
        }

        $this->withToken($kabag)->patchJson("/api/v1/bookings/{$bookingId}/alternative", [
            'alternativeRoomId' => fake()->uuid(),
