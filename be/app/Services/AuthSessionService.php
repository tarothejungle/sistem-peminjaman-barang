...
final class AuthSessionService
{
    ...
    public function create(User $user, bool $recordLogin = false): array
    {
        return DB::transaction(function () use ($user, $recordLogin): array {
    ...
    /**
     * Read-only twin of authenticate(): it answers whether the session is still
     * usable without revoking an inactive one. Middleware that only needs a yes
     * or no must use this, otherwise it changes the error the real jwt
     * middleware reports.
     */
    public function peek(string $sessionId): ?AuthSession
    {
        $session = AuthSession::query()->with('user')->find($sessionId);
    ...
    private function isActive(?AuthSession $session): bool
    {
        if (! $session || ! $session->user || $session->revoked_at || $session->expires_at->isPast()) {
            return false;
        }

        return ! $session->last_activity_at->addSeconds(config('jwt.inactivity_ttl'))->isPast();
    }
