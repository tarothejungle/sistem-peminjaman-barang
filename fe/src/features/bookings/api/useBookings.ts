...
/** Realtime booking views poll on this cadence so pending approvals surface quickly. */
const BOOKING_POLL_MS = 3_000;
...
export interface CreateItemBookingInput extends BookingBaseInput {
  ...
  /** Surat Tugas PDF; mandatory as soon as the loan includes a vehicle. */
  suratTugas?: File;
}
...
export function useBookingAvailabilitySummary(enabled = true) {
  return useQuery({
    ...
    enabled,
    refetchInterval: enabled ? BOOKING_POLL_MS : false,
...
export function useBookingAvailability(input: BookingAvailabilityInput | null) {
  return useQuery({
    ...
    enabled: input !== null,
    refetchInterval: input ? BOOKING_POLL_MS : false,
    refetchOnWindowFocus: true,
    staleTime: 0,
  });
}
...
export function useCreateBooking() {
  ...
  return useMutation({
    mutationFn: async (input: CreateBookingInput): Promise<Booking> => {
      const response = await api.post<ApiResponse<Booking>>("/bookings", bookingFormData(input));
      return response.data.data;
    },
    onSuccess: async () => {
      await Promise.all([
...
/**
 * Every booking is submitted as multipart: a vehicle loan ships its Surat Tugas
 * PDF, and JSON cannot carry a file. Item payloads are flattened into the
 * items[index][field] shape Laravel validates.
 */
function bookingFormData(input: CreateBookingInput): FormData {
  ...
  if (input.resourceType === "ROOM") {
    ...
    formData.append("workUnit", input.workUnit);
    if (input.document) formData.append("document", input.document);
    return formData;
  }

  if (input.suratTugas) formData.append("suratTugas", input.suratTugas);
...
export function useUpdatePendingBooking() {
  ...
  return useMutation({
    mutationFn: async ({ bookingId, input }: { bookingId: string; input: CreateBookingInput }): Promise<Booking> => {
      ...
      const response = await api.post<ApiResponse<Booking>>(`/bookings/${bookingId}`, bookingFormData(input));
      return response.data.data;
    },
...
export function useMyBookings() {
  return useQuery({
    ...
    refetchInterval: BOOKING_POLL_MS,
    refetchOnWindowFocus: true,
  });
}

export function useAllBookings(status?: BookingStatus, enabled = true) {
  return useQuery({
...
export interface RelocateBookingInput {
  bookingId: string;
  alternativeRoomId: string;
  /** Jakarta calendar date (YYYY-MM-DD); the server derives the timestamps. */
  alternativeDate: string;
  /** Session registered in "Pengaturan Jam Ruangan". */
  alternativeRoomSlot: RoomBookingSlot;
}
...
function getStatusUpdateEndpoint(bookingId: string, status: BookingStatus, actorRole?: Role): string {
  ...
  if (status === "APPROVED" || status === "REJECTED") return `/bookings/${bookingId}/kabag-approve`;
  if (status === "COMPLETED") return `/bookings/${bookingId}/pj-inspect`;
  throw new Error("Transisi status tidak didukung");
}

export function useRelocateBooking() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ bookingId, ...input }: RelocateBookingInput): Promise<Booking> => {
      const response = await api.patch<ApiResponse<Booking>>(`/bookings/${bookingId}/alternative`, input);
      return response.data.data;
    },
    onSuccess: async () => queryClient.invalidateQueries({ queryKey: bookingQueryKeys.all }),
  });
